...
const ThreeDMap = observer(() => {
  ...
  const trajectoryMock = [
    [
      [[116.593238, 40.051893], [116.610159, 40.063333]],
      [[116.610159,    40.063333], [116.610275, 40.063344]],
      [[116.610275,    40.063344], [116.610345, 40.063336]]
    ],
    [
      [[116.610345,    40.063336], [116.610466, 40.063304]],
      [[116.610466,    40.063304], [116.610576, 40.063245]]
    ]
  ];
  ...
  useEffect(() => {
    ...
    fetch(geoJsonUrl)
      ...
      .then(data => {
        ...
        // 2. 创建 MapScene：把 Three.js 渲染层挂到地图上
        // const mapScene = new MapScene(map.current, {
        //     antialias      : true,      // 透传给 WebGLRenderer 的配置
        //     alpha          : true,
        //     renderOnMoving : true,       // 地图移动/旋转时自动重渲
        //     autoStart       : true    // ← 确保已经运行
        // });
        
        // function lngLatToVector3(lng, lat, alt = 0) {
        ...
        // // 经纬度 → Three.js 空间坐标
        // const pos = lngLatToVector3(116.593238, 40.051893, 0); // 北京天安门坐标
        // // const pos = mapScene.lngLatToPosition([116.404, 39.915], 0); // 返回 Vector3
        // cube.position.copy(pos);
        
        // mapScene.scene.add(cube);
        // mapScene.scene.add(new THREE.AmbientLight(0xffffff, 0.8));
        
        // // 4. 初始化完毕后，手动渲一次或等待 map 触发 'idle'
        // // mapScene.requestRender();
        // map.current.triggerRepaint();

        // sceneRef.current = new MapScene(map.current, {
        //   antialias: true,
        //   alpha: true,
        //   renderOnMoving: true,   // 拖动时实时渲染
        //   autoStart: true        // 保证渲染循环已跑起来
        // });
        ...
        map.current.on('load', () => {
          ...
          // map.current.moveLayer('three_layer');   // three_layer 永远在最上
    
          /* 3. 把测试轨迹画进去 ------------------------------------------- */
          drawTrajectory('test', trajectoryMock, '#ff0000');


          if (map.current.getLayer('three_layer')) {
            map.current.moveLayer('three_layer');
          }

          // websocketStore.startSimulate(); // 启动模拟
          console.log('[Step 1] map loaded');

          // sceneRef.current = new MapScene(mapRef.current, { autoStart: true });
          sceneRef.current.renderer.domElement.addEventListener('webglcontextlost', e => {
            console.warn('WebGL context lost', e);
          });

          // 关键：MapScene 创建成功时它会发出 custom-layer-added 事件
          map.current.once('custom-layer-added', () => {
            console.log('[Step 1] three_layer 已注入样式 ✅');
          });

          // 如果 2 秒内没等到上面那条日志，就说明 three_layer 没被加进来
          setTimeout(() => {
            if (!map.current.getLayer('three_layer')) {
              alert('❌ MapScene 没有把 three_layer 加进来！请升级插件。');
            }
          }, 2000);
      ...
      websocketStore.planePosition.forEach(plane => {
          ...
          if (!existingPlaneIds.has(id)) {
            ...
            markers.current[id] = { marker }; // 存储新标记和轨迹数据
              ...
              const trajectoryMock = [
                [
                  [[116.61, 40.0633], [116.610159, 40.063333]],
                  [[116.610159, 40.063333], [116.610275, 40.063344]]
                ]
              ];
              
              drawTrajectory('test', trajectoryMock, '#ff0000', 0);
    ...
    const drawTrajectory = (id, trajectorys, color, h) => {
      ...
      if (!sceneRef.current) return;                 // 地图尚未 load
        const scene = sceneRef.current.scene;

      // 如果该飞机之前画过轨迹，先删掉旧组
      if (scene.getObjectByName(`track-${id}`)) {
        scene.remove(scene.getObjectByName(`track-${id}`));
      }

      const trackGroup = new THREE.Group();
      trackGroup.name = `track-${id}`;               // 便于下次删除
      trackGroup.renderOrder = 999; 

      const alt = 5;                                 // 抬高 5 m，避免被盖

      console.log(JSON.stringify(trajectorys))


      const mat = new THREE.LineBasicMaterial({
        color: new THREE.Color(color),
        linewidth : 2,
        depthTest : false,  // 避免被底图盖住
        transparent: true,
        opacity: 0.9,
      });

      // ② 公用：经纬度 → Three Vector3
      function lngLatToV3(lng, lat, alt = 5) {
        const mc = maplibregl.MercatorCoordinate.fromLngLat({ lng, lat }, alt);
        return new THREE.Vector3(mc.x, mc.y, mc.z);
      }

      trajectorys.forEach(segmentGroup => {
        segmentGroup.forEach(twoPointSeg => {
          if (twoPointSeg.length < 2) return;       // 防守
          const pts = twoPointSeg.map(([lng, lat]) => lngLatToV3(lng, lat, alt));
      ...
      // const mapScene = new MapScene(map.current, { antialias: true, alpha: true });

      // const trackGroup = new THREE.Group();
      // const trackAlt   = 1;      // 统一海拔（米）

      // geodata.features.forEach(feature => {
      //   feature.geometry.coordinates.forEach(lineCoords => {
      //     // 把经纬度序列转成 Vector3[]
      //     const points = lineCoords.map(([lng, lat], idx) => {
      //       const m = maplibregl.MercatorCoordinate.fromLngLat({ lng, lat }, trackAlt);
      //       return new THREE.Vector3(m.x, m.y, m.z);
      //     });
        
      //     /* ---------- 3.1 简单线条（性能好） ---------------- */
      //     const lineGeom = new THREE.BufferGeometry().setFromPoints(points);
      //     const lineMat  = new THREE.LineBasicMaterial({ color: 0x00E0FF, linewidth: 4 });
      //     const line     = new THREE.Line(lineGeom, lineMat);
      //     trackGroup.add(line);
        
      //     /* ---------- 3.2 TubeGeometry（若需粗管状效果） ----
      //     const curve    = new THREE.CatmullRomCurve3(points);
      //     const tubeGeom = new THREE.TubeGeometry(curve, 64, 5, 8, false);  // 半径 5 米
      //     const tubeMat  = new THREE.MeshStandardMaterial({ color: 0x0094FF });
